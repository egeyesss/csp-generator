...
from csp_generator.models import Clue, GenerationMetrics, NegativeAssociation, Puzzle, Theme
...
def generate(
    ...
    2. Enumerate every clue that's true under that solution, excluding
       NegativeAssociation (numerous, weak, and never appear in well-crafted
       zebra puzzles — AbsolutePosition already covers their role).
    ...
    `n_restarts` controls how many greedy passes the selector runs; more
    passes means a smaller (but not provably optimal) clue set.
    """
    if rng is None:
        rng = random.Random()
    solution = generate_solution(theme, rng)
    pool: list[Clue] = [
        c for c in enumerate_valid_clues(solution, theme) if not isinstance(c, NegativeAssociation)
    ]
